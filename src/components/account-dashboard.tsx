...
import { emptyAddress, getCustomerProfile, normalizeDefaultAddress, saveCustomerProfile, type CustomerAddress, type CustomerProfile } from "@/lib/firebase/customer";
...
export function AccountDashboard() {
  ...
  async function saveProfile(event: FormEvent) {
    ...
    catch { setNotice("We couldn't save your profile. Please try again."); }
  ...
  async function saveAddress(event: FormEvent) {
    ...
    const next = { ...profile, addresses }; setProfile(next);
    try { await saveCustomerProfile(user.uid, next); setEditing(null); setNotice("Address saved."); }
    catch { setNotice("We couldn't save this address. Please try again."); }
    finally { setBusy(false); }
  }
  async function removeAddress(id: string) {
    if (!user) return; const remaining = profile.addresses.filter((address) => address.id !== id); const wasDefault = profile.addresses.find((address) => address.id === id)?.isDefault; const addresses = wasDefault && remaining[0] ? normalizeDefaultAddress(remaining, remaining[0].id) : remaining; const next = { ...profile, addresses }; setProfile(next); await saveCustomerProfile(user.uid, next); setNotice("Address removed.");
  }
  async function makeDefault(id: string) { if (!user) return; const next = { ...profile, addresses: normalizeDefaultAddress(profile.addresses, id) }; setProfile(next); await saveCustomerProfile(user.uid, next); setNotice("Default address updated."); }
  ...
  return <main className="account-page"><header className="account-hero"><div><p className="eyebrow"><span /> Customer account</p><h1>Welcome{profile.displayName ? `, ${profile.displayName.split(" ")[0]}` : " back"}.</h1><p>{user.email}</p></div><button onClick={() => signOut(firebaseAuth())}>Sign out</button></header>
    <div className="account-layout"><nav className="account-nav" aria-label="Account sections">{([['profile','Profile'],['addresses','Addresses'],['orders','Orders'],['security','Security']] as const).map(([id,label]) => <button className={section === id ? "active" : ""} onClick={() => setSection(id)} key={id}>{label}<span>→</span></button>)}</nav>
      <section className="account-content">{notice && <p className="account-notice" role="status">{notice}</p>}
        ...
        {section === "security" && <div className="account-card"><div className="account-card-head"><div><small>Account access</small><h2>Security</h2></div></div><div className="security-list"><div><span>{user.emailVerified ? "✓" : "!"}</span><p><strong>Email address</strong><small>{user.emailVerified ? "Your email is verified." : "Verify your email to strengthen account recovery."}</small></p>{!user.emailVerified && <button onClick={() => void sendEmailVerification(user).then(() => setNotice("Verification email sent."))}>Send verification</button>}</div><div><span>••</span><p><strong>Password</strong><small>We&apos;ll send a secure reset link to {user.email}.</small></p><button onClick={() => user.email && void sendPasswordResetEmail(firebaseAuth(), user.email).then(() => setNotice("Password reset email sent."))}>Reset password</button></div><div><span>↗</span><p><strong>Privacy request</strong><small>Request a copy or deletion of your account data.</small></p><a href="mailto:hello@printstore.local?subject=Printstore privacy request">Contact privacy</a></div></div></div>}
      </section></div>

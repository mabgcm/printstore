...
export function OrderTracker() {
  ...
  return <div className="tracker-shell">
    ...
    <div className="tracked-orders">{visible.map((order) => { const current = stageFor(order); const cancelled = /cancel/i.test(order.status); return <details className="tracked-order" key={order.id}>
      <summary><span><b>Order {order.displayId}</b><small>{new Intl.DateTimeFormat("en-CA", { dateStyle: "medium" }).format(new Date(order.createdAt))}</small></span><strong>{statusLabel(order)}</strong><span className="order-summary-total">{money(order.total)} CAD</span><i aria-hidden="true">⌄</i></summary>
      <div className="order-detail-heading"><div><p>Order {order.displayId}</p><h2>{statusLabel(order)}</h2><small>Placed {new Intl.DateTimeFormat("en-CA", { dateStyle: "medium", timeStyle: "short" }).format(new Date(order.createdAt))}</small>{order.paymentStatus === "paid" && <em className="order-paid-badge">✓ Payment confirmed</em>}</div><strong>{money(order.total)} CAD</strong></div>

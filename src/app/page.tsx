...
export default async function Home() {
  ...
  return <main>
    ...
    <section id="shop" className="mx-auto max-w-7xl scroll-mt-24 px-6 py-24 sm:px-10 lg:px-16">
      ...
      {products.length ? <div className="mt-10 grid gap-x-7 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">{products.map((product) => { const image = product.images.find((item) => item.is_default) ?? product.images[0]; const variants = product.variants.filter((item) => item.is_enabled && item.is_available); const price = variants.length ? Math.min(...variants.map((item) => item.price)) / 100 : null; return <Link href={`/products/${product.id}`} key={product.id} className="group"><div className="relative aspect-square overflow-hidden rounded-3xl bg-white">{image && <Image src={image.src} alt={product.title} fill sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" className="object-cover transition duration-500 group-hover:scale-105" />}</div><div className="mt-4 flex items-start justify-between gap-4"><div><h3 className="font-bold">{product.title}</h3><p className="mt-1 text-sm text-black/45">Made to order</p></div>{price !== null && <span className="shrink-0 text-sm font-semibold">From ${price.toFixed(2)}</span>}</div></Link>; })}</div> : <div className="mt-10 rounded-3xl bg-white p-12 text-center text-black/55">New products are on the way.</div>}
    </section>
